      ...
      handler: function(after, before) {
        ...
        if (!vm.request_status) {
          ...
          element.innerHTML = `
            ...
            <span>No matching records found ${vm.pagination.filter}</span>
            </div>`;
        } else {
          element.innerHTML = `
            <div class="full-width row flex-center text-accent q-gutter-sm">
            <i aria-hidden="true" class="material-icons q-icon" style="font-size: 2em;">warning</i>
            <span>Data sedang diproses ${vm.pagination.filter}</span>
            </div>`;
        }
      },
      deep: true
    ...
    fetchData() {
      ...
      this.$axios
        ...
        .then(function(response) {
          ...
          // vm.server_paginate.total = response.data.payload.total;

          // console.log(JSON.stringify(vm.server_paginate));
