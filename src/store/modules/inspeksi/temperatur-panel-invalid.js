...
const mutations = {
    ...
    [grid]: (state) => {
        state.config.dynamic.grid = !state.config.dynamic.grid
            // console.log('grid', state.config.dynamic.grid);

    },

    [pagination]: (state, data) => {
        state.data.pagination = data
            // console.log('inspeksi_temperatur_panel_invalid.data.pagination', state.data.pagination);
    },
    [data]: (state, data) => {
        state.data.payload = data
    ...
    [replace]: (state, params) => {
        ...
        if (data.length > 0) {
            // console.log('mutation modules/inspeksi/temperatur_panel_invalid.js ', params);
            for (let i = 0; i < data.length; i++) {
                const element = data[i];
                if (params.uuid == element.uuid) {
                    console.log(params.uuid, element.uuid);
