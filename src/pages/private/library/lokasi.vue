    ...
    <q-page>
      <data-table-library :_store_module="store_module" :_visibleColumnStatus="false">
        ...
        <q-toolbar slot="title" style="padding:0px">
          ...
          <q-toolbar-title>Daftar Lokasi</q-toolbar-title>
        </q-toolbar>
          ...
          <q-tr>
            ...
            <q-td key="name_location" :props="props.props">
              <q-chip v-pointer color="teal" text-color="white" icon="edit">
                <span>{{ props.props.row.name_location ? props.props.row.name_location : data_row('name_location', props.props.row.__index) | upper}}</span>
                <q-tooltip
                  v-if="!isMobileViewport"
                  content-style="font-size: 14px"
                  anchor="center left"
                ...
                <q-popup-edit
                  ...
                  v-model="props.props.row.name_location"
                  @save="onUpdateHandler('name_location', props.props.row)"
                >
                  <template v-slot="{ initialValue, value, emitValue, validate, set, cancel }">
                    <q-input
                      ...
                      @input="onEmptyHandler($event, 'name_location', props.props.row.__index)"
                      @keypress.enter="onUpdateHandler('name_location', props.props.row)"
                      maxlength="50"
                      autofocus
                      counter
                      hint
              ...
              <inner-loading
                :ref="'spinner_name_location_'+props.props.row.__index"
                :_visible="false"
              />
            </q-td>
...
export default {
  ...
  methods: {
    ...
    requestAxios: _.debounce(function(payload) {
      ...
      vm.$refs["spinner_name_location_" + payload.__index]._data.visible = true;

      // console.log(payload);

      vm.$axios
        .put("/api/v1/library/location", payload)
        .then(function(response) {
          // console.log("response", response.data.payload);

          vm.$refs[
            "spinner_name_location_" + payload.__index
          ]._data.visible = false;

          vm.set_aset_location({
            type: "replace",
