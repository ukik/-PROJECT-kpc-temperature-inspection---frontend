<template>
  <fragment>
    ...
    <data-table-panel-invalid-inspeksi :_store_module="store_module">
      <!-- TITLE -->
      <q-toolbar slot="title" style="padding:0px">
        ...
        <q-toolbar-title>
          <q-item-section>
            ...
            <q-item-label caption>Belum Divalidasi</q-item-label>
          </q-item-section>
        </q-toolbar-title>
      </q-toolbar>
      ...
      <template
        slot="mobile-grid"
        slot-scope="props"
        v-if="isMobileViewport && get_inspeksi_temperatur_panel_invalid_grid"
      >
        <transition name="fade">
          ...
          >
            ...
            <q-card-section>
              ...
              <div class="q-table__grid-item-row">
                <div class="q-table__grid-item-title row">Diperbarui</div>
      ...
      <scroll-to-top />
    </data-table-panel-invalid-inspeksi>
...
export default {
  ...
  data() {
    return {
      expandArray: [],
      store_module: "inspeksi_temperatur_panel_invalid" // lihat modules di store/index.js
    };
  },
  methods: {
    /**
    ...
    gotoTemperaturFormHandler(payload, route) {
      ...
      const pagination = this[
        "get_inspeksi_temperatur_panel_invalid_pagination"
      ];
    ...
    actionVuex(key, payload) {
      ...
      switch (key) {
        ...
        case "validate":
          this.requestAxios({
            url: "/api/v1/mutation/inspection/validation?",
            method: "put",
            payload: payload
          });
          // alert("validate")
    ...
    requestAxios: _.debounce(function(params) {
      ...
      // WAJIB
      // digunakan laravel Model untuk menentukan cluster table
      const pagination = vm["get_inspeksi_temperatur_panel_invalid_pagination"];
      // console.log(pagination);
        ...
        .then(function(response) {
          ...
          switch (method) {
            case "delete":
              break;
            case "put":
              // hapus data yang valid di store invalid
              vm.set_inspeksi_temperatur_panel_invalid({
                type: "replace",
                data: response.data.payload
              });

              // pindahkan data valid ke store valid
